    ...
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            */
            lAddEvent['EN'] = '添加新的事件';
            lAddEvent['TR'] = 'Yeni Etkinlik Ekle';
            lAddEvent['ES'] = '添加一个新的事件';
            lAllDay['EN'] = '全天事件';
            lAllDay['TR'] = 'Tüm Gün';
            lAllDay['ES'] = '全天事件';
            lTotalEvents['EN'] = '本月总事件: ';
            lTotalEvents['TR'] = 'Bu Ayki Etkinlik Sayısı: ';
            lTotalEvents['ES'] = '在本月的总活动: ';
            lEvent['EN'] = '今天记事';
            lEvent['TR'] = 'Etkinlik';
            lEvent['ES'] = 'Evento(s)';
            ...
            function prevAddEvent() {
               ...
               // $this.children('.jalendar-wood').stop().animate({'width' : jalendarWoodW}, 200);
                //$this.children('.wood-bottom').stop().animate({'width' : woodBottomW}, 200);
                //close-button
                $close.hide();
            }
            ...
            arrows[1].on('click', function () {
                ...
                $this.find('.this-month').removeClass('sign') 
                calcMonth();
                prevAddEvent();
            });
            ...
            $this.on('click', '.this-month', function () {
                ...
                prevAddEvent();
                var list = $(this).find('.day-list').remove();
                $(this).addClass('selected').css({'background-color': settings.color}).append(
                	'<div class="day-list" >'
					+'<i class="i-dayList-1"></i>'
					+'<ul class="clear">'
					+	'<li><i class="icos i-dayLi-1"></i>快速记事</li>'
					+	'<li><i class="icos i-dayLi-2"></i>快速提醒</li>'
					+'</ul>'
				+'</div>'
                )
//              $this.children('.jalendar-wood, .wood-bottom').animate({width : '338px' }, 200, function() {
...
//              });
				...
				$('.day-list li').click(function(){
					...
					$(this).parents('.day-list').remove();
					$('#winUp > .winUp-in').eq(index).fadeIn(600,function(){
						
						$(this).find('.inpB-ok').click(function(){ // 点击确认时候 增加当前页面提示
							
							$('#winUp > .winUp-in').eq(index).fadeOut(600);
							
							
							var data =  _this.attr('data-date');//点击确认 添加今日事件标记
							
							//$('.this-month[data-date="' + data + '"]').addClass('dong');
							
							$('.this-month[data-date="' + data + '"]').addClass('sign');
							
						})
						
						$(this).find('.inpB-bye').click(function(){  // 点击取消时候 增加当前页面提示
							
							$('#winUp > .winUp-in').eq(index).fadeOut(600);
							
							_this.removeClass('sign')  //点击取消 可以移除今日事件标记
				...
				$('#winUp > .winUp-in').click(function(){
					$(this).fadeOut(600)
				})
				
				$('.m-day-call-wrap .inpB1')
				
				$('#winUp > .winUp-in > div').click(function(){
					return false;  
				})
